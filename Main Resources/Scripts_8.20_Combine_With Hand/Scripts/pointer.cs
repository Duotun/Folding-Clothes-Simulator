...
public class pointer : MonoBehaviour {
    ...
    public static Vector3 hitPoint; // Point where the raycast hits
    private bool shouldTeleport; // True if there's a valid teleport target
    public static bool triggerdownflag = false;
    public static bool triggerupflag = false;
    public static RaycastHit recordhit;
    ...
    void Update()
    {
        ...
        if (Controller.GetPress(SteamVR_Controller.ButtonMask.Touchpad))
        {
            ...
            if (Physics.Raycast(trackedObj.transform.position, transform.forward, out hit, 100, teleportMask))   //this hit is work to find particles
            {
                recordhit = hit;
                hitPoint = hit.point;  // critical vector.
                hitflag = true;
                ShowLaser(hit);
    ...
    private void picked()   //teleport edit to edit
    {
